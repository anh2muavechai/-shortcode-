    ...
    */
    ...
    public function DeleteFile($filespec, $force = false) {
        try {
            ...
            ACWLog::write_file('FSO', sprintf('DeleteFile(%s):%s', $filespec, $e->getMessage()));//Edit LIXD-572 hungtn VNIT 20160912
        }
        return FALSE;
    }
    ...
    public function DeleteFolderCommandLine($folderspec) {
        try {
            ...
            ACWLog::write_file('FSO', sprintf('DeleteFolderCommandLine(%s):%s', $folderspec, $e->getMessage()));//Edit LIXD-572 hungtn VNIT 20160912
            throw $e;

        }
        return FALSE;
    }
    public function DeleteFileCommandLine($filespec) {
        try {
            if ($this->_file->FileExists($filespec)) {
                $comm = sprintf('del  /S /Q "%s"', $filespec);
                $comm_sjis = mb_convert_encoding($comm, 'sjis-win', 'UTF-8');
                $result = exec($comm_sjis, $output, $return_var);
                return TRUE;
            }
        } catch (com_exception $e) {
            ACWLog::write_file('FSO', sprintf('DeleteFileCommandLine(%s):%s', $filespec, $e->getMessage()));//Edit LIXD-572 hungtn VNIT 20160912
            throw $e;
    ...
    public function FileListFull($filespec, $sub = false, $fill = '', $ffill = '')
    {
        ...
        //add start BP_LIXD-696 DAI VNIT-20170905: get file of lastver
        public function FileListLastVer($filespec, $sub = false, $fill = '', $ffill = '')
    {

        $list = $this->FileFolderListLastVer($filespec,$sub,$fill,$ffill);
        $new_list=array();
        $this->get_list($new_list, $list,'');
        return  $new_list;
    }
        public function FileFolderListLastVer($filespec, $sub = false, $fill = '', $ffill = '',$foldername='') {
        ...
        try {
            ...
            if (strpos($foldername, 'head_') !== false) {
                              //get max folder mei_
                            $maxmei=0;
                            foreach ($sublist as $s)
                            {
                            ...
                            foreach ($sublist as $s)
                            {
                                  ...
                                  if($s==$maxverion)
                                  {
                                    ...
                                    $subpath = $this->BuildPath($filespec, $s);
                                    $list[ '/' . $s ] = $this->FileFolderListLastVer($subpath  , $sub,  $fill, $ffill);
                                  }
                            }
                        }
                        else
                        {
                            ...
                            foreach ($sublist as $s)
                            {
                                    ...
                                    $list[ '/' . $s ] = $this->FileFolderListLastVer($subpath  , $sub,  $fill, $ffill,$s);
                            }
                        }
        } catch (com_exception $e) {
            ACWLog::write_file('FSO', sprintf('FileFolderListLastVer(%s, %d, %s, %s):%s', $filespec, $sub, $fill, $ffill, $e->getMessage()));
        }

        return $list;
    }
        //add END BP_LIXD-696 DAI VNIT-20170905
    ...
    public function GetFileName($pathspec) {
        ...
        public function sevenzip_extract($file_path, $folder_unzip, $replace = false) {
            try {
                //Edit start LIXD-665 TinVNIT 04/17/2017
                if($replace == false)
                {
                ...
                {
                    $comm = sprintf(AKAGANE_WEB_PATH . '/7za.exe x "%s" -o"%s" -aoa', $file_path, $folder_unzip);
                }
                //Edit end LIXD-665 TinVNIT 04/17/2017
    ...
    //Add Start LIXD-627 hungtn VNIT 20170110
    public function get_modif_excel($file_path) {
        try{
            //1.copy ->tmp.zip
            //2. extract
            //3. get data modiff
            ...
            // tmp file
            $tmp_folder = uniqid();
            $tmp_folder_main = ACW_TMP_DIR."/download";
            if($this->FolderExists($tmp_folder_main) == FALSE) {
                $this->CreateFolder($tmp_folder_main);
            }

            $tmp_folder_at = $tmp_folder_main."/$tmp_folder";
            $this->CreateFolder($tmp_folder_at);

            $tmp_file = $tmp_folder_at."/tmp.zip";

            $this->CopyFile($file_path, $tmp_file);

            //===== 2 ======
            if($this->sevenzip_extract($tmp_file, $tmp_folder_at) == FALSE) {
                return FALSE;
            }

            //===== 3 ======
            $xml_file = $tmp_folder_at."/docProps/core.xml";
            if($this->FileExists($xml_file) == FALSE) {
                return FALSE;
            }
            ...
            //===== 4 ======
            $this->DeleteFolderCommandLine($tmp_folder_at);
    ...
    public function update_modif_excel($file_path, $modif_data) {
        try{
            //1.copy ->tmp.zip
            //2. extract
            //3. update data modiff
            //4. zip
            //5. replace excel file
            //6. clear
            ...
            file_put_contents($xml_file, $modif_data);

            //===== 4 ======
            $this->DeleteFile($tmp_file);

            $zip_file = $tmp_file;
            $folder_list = $this->FolderList($tmp_folder_at);
            if(count($folder_list) > 0) {
                foreach($folder_list as $folder_item) {
                    $this->sevenzip_compress($zip_file,$tmp_folder_at . "/" . $folder_item);
                }
                $this->sevenzip_compress($zip_file,$tmp_folder_at . "/[Content_Types].xml");
            } else {
                return FALSE;
            }

            //===== 5 ======
            if($this->CopyFile($zip_file, $file_path) == FALSE) {
                return FALSE;
            }
    ...
    }//Add End LIXD-627 hungtn VNIT 20170110
}
